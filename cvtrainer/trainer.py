...
class Trainer:
    ...
    def train_step(self, batch: Dict[str, torch.Tensor]):
        ...
        meters_values = self._flatten_meters(self.meters)
    ...
    def _update_meters(self, outputs: torch.Tensor, targets: torch.Tensor, loss: float):
        ...
        for meter_name, meter in self.meters.items():
            if hasattr(meter, "update"):
                meter_class_name = meter.__class__.__name__

                if "Accuracy" in meter_class_name:
                    if targets.dim() > 1 and targets.size(1) > 1:
                        targets = targets.argmax(dim=1)
                    meter.update(outputs, targets)
                elif "Loss" in meter_class_name:
                    meter.update(loss)
                elif "SegmentationIoU" in meter_class_name:
                    meter.update(outputs, targets)
                elif any(x in meter_class_name for x in ["F1", "Precision", "Recall"]):
                    if targets.dim() > 1 and targets.size(1) > 1:
                        targets = targets.argmax(dim=1)
                    meter.update(outputs, targets)
                elif "DetectionMap" in meter_class_name:
                    pass

    def _flatten_meters(self, meters: Dict[str, BaseMeter]) -> Dict[str, float]:
        """
        展平 meter 返回值，处理单个数值和字典两种情况

        Args:
            meters: meter 字典 {meter_name: meter_instance}

        Returns:
            展平后的 metrics 字典，所有 value 都是 float 类型

        展平策略（智能判断）:
            1. 如果 meter 返回单个数值，直接使用 meter_name 作为 key
            2. 如果 meter 返回字典：
               - 如果 key 以 meter_name 开头（如 "map_50" for meter "map"），直接使用 key
               - 否则拼接为 "{meter_name}/{key}"
        """
        result = {}
        ...
        for meter_name, meter in meters.items():
            ...
            if isinstance(value, Number):
                result[meter_name] = float(value)
            elif isinstance(value, dict):
                for key, val in value.items():
                    if isinstance(val, Number):
                        if key.startswith(meter_name):
                            result[key] = float(val)
                        else:
                            result[f"{meter_name}/{key}"] = float(val)
            else:
                raise ValueError(
                    f"Meter {meter_name}.get_value() 应返回 Number 或 Dict[str, Number], "
                    f"实际返回类型: {type(value)}"
                )
