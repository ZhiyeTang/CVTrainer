...
class TestBuildOptimizer:
    ...
    def test_build_optimizer_adam(self, small_model):
        """测试 Adam 优化器创建"""
        config = {"type": "Adam", "kwargs": {"lr": 0.001}}
    ...
    def test_build_optimizer_sgd(self, small_model):
        """测试 SGD 优化器创建"""
        config = {"type": "SGD", "kwargs": {"lr": 0.01, "momentum": 0.9}}
        optimizer = build_optimizer(small_model, config)
    ...
    def test_build_optimizer_with_layerwise(self, small_model):
        ...
        config = {
            "type": "Adam",
            "kwargs": {"lr": 0.001},
            "layerwise": {"backbone": 0.0001, "adapter": 0.001, "head": 0.01},
        }
        optimizer = build_optimizer(small_model, config)
...
class TestBuildScheduler:
    ...
    def test_build_scheduler_cosine(self, simple_trainer):
        ...
        config = {"type": "CosineAnnealingLR", "kwargs": {"T_max": 100}}
        scheduler = build_scheduler(simple_trainer.optimizer, config)

        assert scheduler is not None
        assert isinstance(
            scheduler,
            type(torch.optim.lr_scheduler.CosineAnnealingLR(simple_trainer.optimizer, T_max=100)),
        )

    def test_build_scheduler_step(self, simple_trainer):
        """测试 StepLR 调度器创建"""
        config = {"type": "StepLR", "kwargs": {"step_size": 30, "gamma": 0.1}}
        scheduler = build_scheduler(simple_trainer.optimizer, config)
...
class TestBuildHooks:
    ...
    def test_build_hooks_with_logger(self):
        """测试带 LoggerHook 的构建"""
        config = {"hooks": [{"type": "cvtrainer.hooks.logger.LoggerHook", "kwargs": {}}]}
        hooks = _build_hooks(config)
